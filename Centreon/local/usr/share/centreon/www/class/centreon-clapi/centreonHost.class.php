...
class CentreonHost extends CentreonObject
{
	...
	/**
	 * Display all hosts and their templates list
	 *
     * @param string $parameters
     * @return void
	 */
	 public function showWithTpl($parameters = null)
    {
		$sql = "SELECT htr.host_host_id, h.host_name, h.host_alias, h.host_address, h.host_activate, group_concat(tpl.host_name) AS templates
                                FROM host_template_relation htr, host h, host tpl
                                WHERE h.host_id = htr.host_host_id
								AND htr.host_tpl_id = tpl.host_id
								GROUP BY host_host_id ORDER BY host_host_id";
        
		$res = $this->db->query($sql);
		
		echo "id;name;alias;address;activate;templates\n"; 
		
		while ($elem = $res->fetch()){
			echo $elem['host_host_id'] . $this->delim . $elem['host_name'] . $this->delim . $elem['host_alias'] . $this->delim . $elem['host_address'] . $this->delim . $elem['host_activate'] . $this->delim . $elem['templates'] . "\n";
        }
	}
