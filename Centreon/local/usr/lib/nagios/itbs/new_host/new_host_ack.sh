...
###
clapi_cmd='centreon -u admin -p $centreon_admin_password -o HOST -a SHOW | grep "^$host_id;"'
...
echo $ftp_temp_config_dir/$host_name-$host_ip_address.ini >> $logfile
...
clapi_cmd='centreon -u admin -p $centreon_admin_password -o SERVICE -a del -v "$host_name;new_host_ack"'
error_message=$(eval $clapi_cmd)

if [ ! $? ] ; then
	echo "Erreur CLAPI : impossible de dsupprimer le service new_host_ack" >&2
	exit 3
fi
...
error_message=$(sudo -u apache centreon -u admin -p $centreon_admin_password -a POLLERGENERATE -v 1)
if [ $? -eq 1 ] ; then
	# echo centreon -u admin -p $centreon_admin_password -a POLLERGENERATE -v 1 | tee -a $logfile
...
error_message=$(sudo -u apache centreon -u admin -p $centreon_admin_password -a POLLERTEST -v 1)
if [ $? -eq 1 ] ; then
	echo $error_message | tee -a $logfile
...
error_message=$(sudo -u apache centreon -u admin -p $centreon_admin_password -a CFGMOVE -v 1)
if [ $? -eq 1 ] ; then
	echo $error_message | tee -a $logfile
...
error_message=$(sudo -u apache centreon -u admin -p $centreon_admin_password -a POLLERRELOAD -v 1)
if [ $? -eq 1 ] ; then
	echo $error_message | tee -a $logfile
